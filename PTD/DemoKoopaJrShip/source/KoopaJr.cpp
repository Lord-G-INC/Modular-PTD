...
void KoopaJr::init(const JMapInfoIter& rIter) {
    ...
    MR::makeQuatRotateDegree(&mTurnDir, mRotation);
    setInitPose();
    caps.mInitLodCtrlNPC = true;
    caps.mInitLightCtrl = true;
    caps.mInitSound = true;
    caps.mSoundObjsCount = 4;
    caps.mInitHitSensor = true;
    caps.mBodySensorRadius = 175.0f;
    caps.mBodySensorOffset.y = 175.0f;

    caps.mInitSpine = true;
    caps.mWaitNerve = &NrvKoopaJr::HostTypeWait::sInstance;
    caps.mInitTalkCtrl = true;
    caps.mInitSphereShadow = true;
    caps.mSphereShadowRadius = 100.0f;
    caps.mInitEffectKeeper = true;
    mParam.setSingleAction("Wait");
    mParam._0 = 0;
    mParam._1 = 0;
    _13C = "Damage";
    initialize(rIter, caps, "KoopaJr", nullptr, false);
...
namespace NrvKoopaJr {
    ...
    HostTypeReaction(HostTypeReaction::sInstance);
    HostTypeReactionEnd(HostTypeReactionEnd::sInstance);
    HostTypeShipBattleAppear(HostTypeShipBattleAppear::sInstance);
    HostTypeShipBattlePowerUp(HostTypeShipBattlePowerUp::sInstance);
    HostTypeShipBattleEscape(HostTypeShipBattleEscape::sInstance);
    HostTypeShipBattleShipDamage(HostTypeShipBattleShipDamage::sInstance);
    HostTypeShipBattleDemoTalkStart(HostTypeShipBattleDemoTalkStart::sInstance);
    HostTypeShipBattleDemoTalkWait(HostTypeShipBattleDemoTalkWait::sInstance);
};  // namespace NrvKoopaJr