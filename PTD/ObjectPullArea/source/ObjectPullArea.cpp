...
bool ObjectPullConfig::isPullItem (MarioActor *pMarioActor, u32 sensorType) {
	if (!gIsEnabled)
		return false;
	if (!((MarioActor *)pMarioActor->_1080)->isActionOk("�J������")) 
		return false;
	for (s32 i = 0; i < 7; i++) {
...
{
	if (isActionOk((MarioActor *)pMarioActor->_1080, "�R�C����������"))
	{
		u32 sensorType = pSensor->mSensorType;
		if (!strcmp(MR::getSensorHost(pSensor)->mName, "�J���b�N�r�[���p�J��")) 
			sensorType = 5555;
		if (sensorType - 18 > 1) {
			if (sensorType != ATYPE_STAR_PIECE) {
				...
				if (sensorType != 5555) 
					return;
				if (isActionOk((MarioActor *)pMarioActor->_1080, "�J������") && pSensor->receiveMessage(ACTMES_IS_PULL_ENABLE, pMarioActor->getSensor("Body"))) {
		...
		} else if (isActionOk((MarioActor *)pMarioActor->_1080, "�J������") && pSensor->receiveMessage(ACTMES_IS_PULL_ENABLE, pMarioActor->getSensor("Body"))) {
