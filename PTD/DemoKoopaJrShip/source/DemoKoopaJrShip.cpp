...
void DemoKoopaJrShip::init(const JMapInfoIter& rIter) {
    ...
    initModelManagerWithAnm("KoopaJrShip", nullptr, nullptr, false);
    initAnimID(rIter);
    MR::connectToSceneNpc(this);
    initEffectKeeper(0, nullptr, false);
    initSound(8, false, &mTranslation, 0);
    MR::invalidateClipping(this);
    createKoopaJrObj(rIter);
    MR::tryRegisterDemoCast(this, rIter);
    initNerve(&NrvDemoKoopaJrShip::DemoKoopaJrShipNrvAppear::sInstance, 0);
    makeActorDead();
}

void DemoKoopaJrShip::appear() {
    LiveActor::appear();
    setNerve(&NrvDemoKoopaJrShip::DemoKoopaJrShipNrvAppear::sInstance);
    mKoopaJrObj->mTranslation.set(sKoopaJrPos);
}
...
void DemoKoopaJrShip::exeAppear() {
    ...
    if (MR::isBckStopped(this)) {
        setNerve(&NrvDemoKoopaJrShip::DemoKoopaJrShipNrvTalk::sInstance);
    } else if (tryDemoEnd()) {
        return;
    }
}
...
void DemoKoopaJrShip::exeTalk() {
    ...
    if (!DemoFunction::isPauseTimeKeepDemo(this)) {
        setNerve(&NrvDemoKoopaJrShip::DemoKoopaJrShipNrvFlyAway::sInstance);
    } else if (tryDemoEnd()) {
        return;
    }
}
...
void DemoKoopaJrShip::control() {
    ...
    jointMtx.mult(pos, mKoopaJrObj->mTranslation);
}
...
namespace NrvDemoKoopaJrShip {
    DemoKoopaJrShipNrvAppear(DemoKoopaJrShipNrvAppear::sInstance);
    DemoKoopaJrShipNrvTalk(DemoKoopaJrShipNrvTalk::sInstance);
    DemoKoopaJrShipNrvFlyAway(DemoKoopaJrShipNrvFlyAway::sInstance);
}